...
jQuery(function () {
    ...
    jQuery("button.retryTies").bind("click", function () {
        jQuery("tr:has(td.tie) td.test").benchmark()
    });

    jQuery("button.selectAll").bind("click", function () {
        jQuery("input[type=checkbox]").each(function () {
            this.checked = true
        })
    });
    jQuery("button.deselectAll").bind("click", function () {
        jQuery("input[type=checkbox]").each(function () {
            this.checked = false
        })
    });
    ...
    jQuery("#addTest").bind("click", function () {
        ...
        jQuery("div#time-test > button").each(function () {
            this.disabled = true;
        });
        jQuery("tbody tr:last button").bind("click", function () {
            ...
            jQuery("button", td).bind("click", function () {
...
jQuery.fn.max = function (tolerance, percentage) {
    return this.maxmin(tolerance, "max", percentage)
};
jQuery.fn.min = function (tolerance, percentage) {
    return this.maxmin(tolerance, "min", percentage)
};

function withinTolerance(number, target, tolerance, percentage) {
    if (percentage) {
        var high = target + ((tolerance / 100) * target);
        var low = target - ((tolerance / 100) * target);
    }
    else {
        var high = target + tolerance;
        var low = target - tolerance;
    }
